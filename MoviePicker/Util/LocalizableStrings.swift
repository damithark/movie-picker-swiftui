//
//  LocalizableStrings.swift
//  MoviePicker
//
//  Created by Damitha Raveendra on 2024-03-12.
//

import Foundation

enum LocalizableStrings {
    static let appName = NSLocalizedString("Movie Picker", comment: "Movie Picker")
    static let welcomeMessage = NSLocalizedString("Welcome to movie picker", comment: "Welcome to movie picker")
    static let homeNavTitle = NSLocalizedString("Movie Picker", comment: "Movie Picker")
    static let trendingTitle = NSLocalizedString("Trending", comment: "Trending")
    static let searchTitle = NSLocalizedString("Search Movies", comment: "Search Movies")
}
